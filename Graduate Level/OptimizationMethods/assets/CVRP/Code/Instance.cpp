...
void node::print(){
    ...
    cout<<T<<_id<<"("<<_x<<", "<<_y<<", "<< _demand <<")";
    
}

Instance::Instance(string filename)
{
    _knownsolpath = ORUtils::extractpath(filename) + "\\" + ORUtils::extractfilename(filename) + ".sol";
    _name=filename;
    ...
    string name = ORUtils::getData(input, "NAME"); 
    string type = ORUtils::getData(input, "TYPE");
    string comment = ORUtils::getData(input, "COMMENT");
    string dimension = ORUtils::getData(input, "DIMENSION");
    string edge_weight_type = ORUtils::getData(input, "EDGE_WEIGHT_TYPE");
    string edge_weight_format = ORUtils::getData(input, "EDGE_WEIGHT_FORMAT");
	string capacity = ORUtils::getData(input, "CAPACITY");
    ...
    if(name=="n/a" || type=="n/a" || comment=="n/a" || dimension=="n/a" || edge_weight_type=="n/a" || capacity=="n/a") {
    ...
    _Nodes=new node*[_nnodes+1]; //In case we need a virtual final depot
	_Q = stod(capacity);

    bool hasCoords=true;
    if(edge_weight_type=="EXPLICIT"){
        if(!ORUtils::gotoSection(input, "DISPLAY_DATA_SECTION")){
            hasCoords=false;
        }
    }
    else if(edge_weight_type=="EUC_2D" || edge_weight_type=="CEIL_2D" || edge_weight_type=="ATT" || edge_weight_type=="GEO"){
        if(!ORUtils::gotoSection(input, "NODE_COORD_SECTION")){
	...
	if (!ORUtils::gotoSection(input, "DEMAND_SECTION")) {
		throw runtime_error("Could not find DEMAND_SECTION in file " + filename);
	}
	...
	for (int i = 0; i < _nnodes; i++) {
		...
		double demand;
		input >> id >> demand;
		assert(id == _Nodes[i]->id());
		_Nodes[i]->_demand = demand;
	}

	double TotalDemand = accumulate(_Nodes, _Nodes + _nnodes, 0.0, [](double sum, node* n) {return sum + n->demand(); });

	_p = (int)ceil(TotalDemand / _Q);


	if (!ORUtils::gotoSection(input, "DEPOT_SECTION")) {
		throw runtime_error("Could not find DEPOT_SECTION in file " + filename);
	}
	int id;
	input >> id;
	_InitialDepotId = id-1;
	assert(_Nodes[_InitialDepotId]->id() == id);
	_Nodes[id - 1]->_type = NodeType::InitialDepot;
	input >> id;
    assert(id < 0);
	_FinalDepotId = -id-1;
	assert(_Nodes[-id - 1]->id() == -id);
    if (_FinalDepotId == _InitialDepotId) {
		_FinalDepotId = _nnodes;
		_nnodes++;
		_Nodes[_FinalDepotId] = new node(_Nodes[_InitialDepotId]->id(), _Nodes[_InitialDepotId]->_x, _Nodes[_InitialDepotId]->_y, 0, NodeType::FinalDepot);
    }
	else _Nodes[id - 1]->_type = NodeType::FinalDepot;
    
}

void Instance::readExplicit(ifstream& input, string edge_weight_format)
{
    if(!ORUtils::gotoSection(input, "EDGE_WEIGHT_SECTION")){
        throw runtime_error("Could not find EDGE_WEIGHT_SECTION in file "+ name());
    }
...
void Instance::print()
{
	...
	cout << "Capacity= " << _Q << endl;
	cout << "Fleet Size= " << _p << endl;
...
double Instance::travellingtime(node* Ni, node* Nj){
    ...
    else if (_DM == DistanceMode::Geographical)
    {
        ...
        double lat_i = ORUtils::coordtogeo(Ni->_x);
        double lng_i = ORUtils::coordtogeo(Ni->_y);

        double lat_j = ORUtils::coordtogeo(Nj->_x);
        double lng_j = ORUtils::coordtogeo(Nj->_y);
...
double Instance::length(vector<int>& tour)
{
	double length = 0;
	for (int i = 0; i < tour.size(); i++)
	{
		length += travellingtime(Node(tour[i]), Node(tour[(i + 1) % tour.size()]));
	}
	return length;
